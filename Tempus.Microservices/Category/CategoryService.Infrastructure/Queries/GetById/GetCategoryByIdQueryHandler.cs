...
public class GetCategoryByIdQueryHandler(CategoryServiceDbContext context)
    : IRequestHandler<GetCategoryByIdQuery, BaseResponse<BaseCategory>>
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            if(request.GroupId.HasValue && category.OwnerId != request.GroupId.Value &&
               request.GroupId.Value != Guid.Empty && category.OwnerId != request.UserId)
            {
